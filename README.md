...
Use Python 3.12 and uv. Place the Models and SSL repository checkouts next to each other:
...
The relative Models dependency is deliberate and confined to development installation. `uv build` emits a wheel depending on `smi-research-models==0.1.0`, without a workspace path. For arbitrary installation locations, install the Models wheel and SSL wheel together with pip/uv pip. No original research checkout is needed.
...
6. **Optimize and validate.** The retained B0 objective is masked waveform MSE; original derivative/energy loss components and B1–B3 settings remain available explicitly in the configuration. Validation compares against zero, visible mean, nearest-neighbour and linear-interpolation baselines. The original loop's standard validation policy is P25; `evaluate_reconstruction(..., evaluation_policy="CYCLIC25")` explicitly evaluates complete cycles. Training and validation masks therefore have distinct, documented roles.
...
Provide rows for all three splits `train`, `val` and `real_val`. The single row above only illustrates the schema and is not a complete dataset. Bounds use source sample indices, `[start_sample,end_sample)`; time is converted using 2 MHz. `path` is relative to the supplied signal root. Labels are integer class IDs for downstream interpretation; they do not enter the reconstruction loss. Use `-1` if labels are unavailable (training works, class-mapped clustering then requires labels). A `group` must identify the whole original acquisition/noise carrier; all its derived events belong to one split. If omitted, the source file checksum is used as the group.
